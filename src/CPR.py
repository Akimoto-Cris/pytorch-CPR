...
def blockFactory(inp_feats, output_feats, blocktype="standard", stage1=False, kernel_size=3, activation=False):
    if stage1:
        ...
        if blocktype == "standard":
            ...
            post_block = nn.ModuleList([
                nn.Sequential(
                    Bottleneck(inp_feats, output_feats, activation, kernel_size, 1),
                    Bottleneck(output_feats, output_feats, activation, kernel_size, 1)
                ),
                nn.Sequential(
                    make_standard_block(inp_feats, output_feats, 1, 1, 0),
                    Bottleneck(output_feats, output_feats, activation, 1, 1)
                )
            ])
            return nn.ModuleList([nn.Sequential(*layers), post_block])
        else:
    ...
    def __init__(self, backend, backend_outp_feats, n_joints, n_paf, n_stages=7,
        ...
        if not share:
            layers += [Stage(backend_outp_feats, n_joints, n_paf, False, blocktype=blocktype,
                             kernel_size=kernel_size, activation=activation)] * (n_stages - 2)
...
class Stage(nn.Module):
    def __init__(self, backend_outp_feats, n_joints, n_paf, stage1, blocktype="standard", kernel_size=3, activation=False):
        ...
        self.stage1 = stage1
        if self.stage1:
            self.block1 = blockFactory(backend_outp_feats, n_joints, blocktype, self.stage1,
                                       kernel_size=kernel_size, activation=activation)
            self.block2 = blockFactory(backend_outp_feats, n_paf, blocktype, self.stage1 ,
                                       kernel_size=kernel_size, activation=activation)
        else:
            inp_feats = backend_outp_feats + n_joints + n_paf
            self.block1 = blockFactory(inp_feats, n_joints, blocktype, self.stage1,
                                       kernel_size=kernel_size, activation=activation)
            self.block2 = blockFactory(inp_feats, n_paf, blocktype, self.stage1,
                                       kernel_size=kernel_size, activation=activation)
        init(self.block1)
        init(self.block1)
    ...
    def forward(self, x):
        if self.stage1:
            ...
            y1 = self.block1[0](x)
            y2 = self.block2[0](x)
            y1 = self.block1[1][0](y1) + self.block1[1][1](y1)
            y2 = self.block2[1][0](y2) + self.block2[1][1](y2)
        return y1, y2
