...
class CocoDataSet(data.Dataset):
    def __init__(self, data_path, opt, split='train'):
        ...
        if split is not "test":
            ...
            self.coco = COCO(os.path.join(data_path, 'annotations/image_info_test{}.json'.format(self.coco_year)))
            self.indices = sorted(self.coco.getImgIds())
    ...
    def get_item_raw(self, index, resize_factor=None):
        to_resize = resize_factor is not None
        out_shape = tuple([int((resize_factor if resize_factor else 1) * self.opt[self.split]["imgSize"])] * 2)
        ...
        if self.split in ["train", "val"]:
            ...
            if to_resize:
                img, ignore_mask, keypoints = resize(img, ignore_mask, keypoints, out_shape)
    ...
    def __getitem__(self, index):
        if self.split in ["train", "val"]:
            img, heat_map, paf, ignore_mask, _ = self.get_item_raw(index, 1)
            img = normalize(img)
    ...
    def get_imgs_multiscale(self, index, scales, resize_factor=None, flip = False):
        # img, heat_map, paf, ignore_mask, _ = self.get_item_raw(index, resize_factor=resize_factor)
        img, heat_map, paf, ignore_mask, keypoints = self.get_item_raw(index, resize_factor=resize_factor)
        imgs = []
        for scale in scales:
        ...
        # return imgs, heat_map, paf, ignore_mask, imgs[0].shape
        return imgs, heat_map, paf, ignore_mask, imgs[0].shape, keypoints

    def get_imgs_multiscale_resize(self, index, scales, flip=False, resize_factor=None):
        if self.split in ["train", "val"]:
            img, heat_map, paf, ignore_mask, _ = self.get_item_raw(index, resize_factor=resize_factor)
            img_temp, _, _, _, _ = self.get_item_raw(index, resize_factor=None)
            imgs = []
            for scale in scales:
                width, height = img.shape[1], img.shape[0]
                new_width, new_height = int(scale * width), int(scale * height)
                ori_img, _, _, _, _ = self.get_item_raw(index, resize_factor=resize_factor)
                scaled_img = cv2.resize(ori_img.copy(), (new_width, new_height))
                flip_img = cv2.flip(scaled_img, 1)
                scaled_img = normalize(scaled_img)
                imgs.append(scaled_img)
                if flip:
            ...
            img = self.get_item_raw(index, resize_factor=resize_factor)
            img_temp = self.get_item_raw(index, resize_factor=None)
            imgs = []
            for scale in scales:
                width, height = img.shape[1], img.shape[0]
                new_width, new_height = int(scale * width), int(scale * height)
