...
class Consumer(mp.Process):
    ...
    def do_some_works(self, query):
        ...
        if isinstance(self.indices, list):
            self.indices.append(self.dataset.indices[:self.dataset_len])
        else:
...
def eval_net(dataloader, model, opts):
    ...
    with tqdm(total=dataset_len) as t:
        for i in range(dataset_len):
            ...
            print(img.shape)
            img_batch = np.zeros((1, 3, opts["val"]["imgSize"], opts["val"]["imgSize"]))
            img_batch[0, :, :, :] = img
            with torch.no_grad():
                imgs_torch = torch.from_numpy(img_batch).float().cuda()
                heatmaps, pafs = model(imgs_torch)
                heatmap = heatmaps[0].data[-1].cpu().numpy()
                paf = pafs[0].data[-1].cpu().numpy()
