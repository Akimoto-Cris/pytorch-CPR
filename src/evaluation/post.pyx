...
cdef tuple[:] joint_to_limb_heatmap_relationship = BODY_PARTS
...
cdef int[:, :] find_peaks(dict param, float[:, :] img):
    """
...
@cython.boundscheck(False)
@cython.wraparound(False)
cdef float[:] compute_resized_coords(int[:] coords, float resizeFactor):
    """
...
@cython.boundscheck(False)
@cython.wraparound(False)
cdef list NMS(dict param, float[:, :, :] heatmaps, float upsampFactor=1.0,
        bint bool_refine_center=True, bint bool_gaussian_filt=False):
    """
    ...
    cdef list joint_list_per_joint_type = []
    cdef int cnt_total_joints = 0
    cdef float[:, :] map_orig, peaks, patch, map_upsamp
    cdef int[:, :] peak_coords
    cdef int x_min, y_min, x_max, y_max
    cdef int[:] location_of_max
    cdef float[:] refined_center, location_of_patch_center, temp_cords
    cdef float peak_score
    ...
    for joint in range(NUM_JOINTS):
        ...
        for i, peak in enumerate(peak_coords):
            if bool_refine_center:
                ...
                # maximum is
                refined_center = np.array([m - c for m, c in zip(location_of_max, location_of_patch_center)])
                peak_score = map_upsamp[location_of_max[0], location_of_max[1]]
            else:
                refined_center = np.zeros((1,2))
                # Flip peak coordinates since they are [x,y] instead of [y,x]
                peak_score = map_orig[peak[1], peak[0]]
            temp_cords =[a + b for a, b in zip(compute_resized_coords(peak_coords[i], upsampFactor), refined_center[::-1])]
            peaks[i, :] = tuple([int(round(x)) for x in temp_cords]) + (peak_score, cnt_total_joints)
            cnt_total_joints += 1
...
@cython.boundscheck(False)
@cython.wraparound(False)
cdef float[:, :, :] find_connected_joints(dict param, float[:, :, :] paf_upsamp,
    ...
    """
    cdef list connected_limbs = []       # limb_type x num_all_limbs x 5
    cdef float[:, :] joints_src, joints_dst
    cdef list connection_candidates = []
    cdef int[:, :] limb_dir, connections
    cdef int limb_dist, max_connections, i, j
    cdef float[:, :, :] intermed_paf, score_intermed_pts
    cdef float score_penalizing_long_dist, criterion1, s
    cdef bint criterion2


    # Auxiliary array to access paf_upsamp quickly
    cdef int[:, :] limb_intermed_coords = np.empty((4, num_intermed_pts), dtype=np.intp)
        ...
        if len(joints_src) == 0 or len(joints_dst) == 0:
            ...
            for i, joint_src in enumerate(joints_src):
                ...
                for j, joint_dst in enumerate(joints_dst):
                    ...
                    # of limb_dir)
                    limb_dist = np.sqrt(np.sum(np.power(limb_dir, 2))) + 1e-8
                    limb_dir = np.divide(limb_dir, limb_dist)  # Normalize limb_dir to be a unit vector
                    ...
                    intermed_paf = np.asarray(paf_upsamp)[limb_intermed_coords[0, :],
            ...
            for potential_connection in connection_candidates:
                i, j = potential_connection[0:2]
                s = potential_connection[3]
...
@cython.boundscheck(False)
@cython.wraparound(False)
cdef int[:, :] group_limbs_of_same_person(float[:, :, :] connected_limbs, float[:, :] joint_list):
    """
    ...
    """
    # matched edges
    cdef list person_to_joint_assoc = [] # shape == N_person x (n_Joint + 2)
    cdef list person_assoc_idx = []
    cdef int joint_src_type, joint_dst_type
    cdef int[:] person_limbs, person1_limbs, person2_limbs, row
    ...
    for limb_type in range(NUM_LIMBS):
        ...
        for limb_info in connected_limbs[limb_type]:
            ...
            if len(person_assoc_idx) == 1:
                ...
                membership = ((np.asarray(person1_limbs) >= 0) & (np.asarray(person2_limbs) >= 0))[:-2]
                if not membership.any():  # If both people have no same joints connected, merge them into a single person
                    ...
                    person1_limbs[:-2] = (np.asarray(person2_limbs)[:-2] + 1) + person1_limbs[:-2]
                    # Update the overall score and total count of joints
                    # connected by summing their counters
                    person1_limbs[-2:] = np.asarray(person2_limbs)[-2:] + np.asarray(person1_limbs)[-2:]
...
def decode_pose(float[:, :, :] img_orig, dict param, float[:, :, :] heatmaps, float[:, :, :] pafs):
    ...
    cdef int[:, :] person_to_joint_assoc = group_limbs_of_same_person(connected_limbs, joint_list)
...
@cython.boundscheck(False)
@cython.wraparound(False)
cdef append_result(int image_id, int[:, :] person_to_joint_assoc, float[:, :] joint_list,
                    dict[:] outputs):
    ...
    cdef dict one_result
    cdef float[:, :] keypoints
    cdef int ind, index
