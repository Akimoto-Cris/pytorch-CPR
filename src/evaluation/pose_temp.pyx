...
cpdef double[: ,:] NMS(dict param, double[:, :, :] heatmaps, float upsampFactor=1.,
    ...
    for joint in range(NUM_JOINTS):
        ...
        cdef object[np.intp, ndim=2] peaks = np.zeros((len(peak_coords), 4))
        for i, peak in enumerate(peak_coords):
            if bool_refine_center:
                ...
                cdef double peak_score = map_orig[tuple(peak[::-1])]
...
cpdef double[:, :, :] find_connected_joints(dict param, double[:, :, :] paf_upsamp,
                                            int[:, :, :, :] joint_list_per_joint_type, int num_intermed_pts=10):
    """
    ...
    for limb_type in range(NUM_LIMBS):
        ...
        cdef int[:, :, :] joints_src = joint_list_per_joint_type[joint_to_limb_heatmap_relationship[limb_type][0]]
        # List of all joints of type B found, where B is specified by limb_type
        # (eg: a right forearm ends in a right wrist)
        cdef int[:, :, :] joints_dst = joint_list_per_joint_type[joint_to_limb_heatmap_relationship[limb_type][1]]
        if len(joints_src) == 0 or len(joints_dst) == 0:
            ...
            cdef object[list, ndim=2] connected_limbs = []
            connected_limbs.append([])
        else:
            cdef double[:, :] connection_candidates = [[]]
            ...
            for i, joint_src in enumerate(joints_src):
                ...
                for j, joint_dst in enumerate(joints_dst):
                    ...
                    cdef double[:, :, :] intermed_paf = paf_upsamp[limb_intermed_coords[0, :],
                                              limb_intermed_coords[1, :], limb_intermed_coords[2:4, :]].T

                    cdef double[:, :, :] score_intermed_pts = intermed_paf.dot(limb_dir)/(np.linalg.norm(intermed_paf)+1e-8)

                    cdef double score_penalizing_long_dist = score_intermed_pts.mean() + \
                                                             min(0.5 * paf_upsamp.shape[0] / limb_dist - 1, 0)
                    # Criterion 1: At least 80% of the intermediate points have
                    # a score higher than thre2
                    cdef double criterion1 = (np.count_nonzero(score_intermed_pts > param['thre2']) > 0.8 * num_intermed_pts)
            ...
            for potential_connection in connection_candidates:
                ...
                cdef double s = potential_connection[3]
...
cpdef group_limbs_of_same_person(double[:, :, :] connected_limbs, int[:, :, :] joint_list):
    """
    ...
    """
    cdef list person_to_joint_assoc = []
...
cpdef decode_pose(double[:, :, :] img_orig, dict param, double[:, :, :] heatmaps, double[:, :, :] pafs):
    heatmaps = heatmaps.transpose(1,2,0)
    pafs = pafs.transpose(1,2,0)
    # Bottom-up approach:
    # Step 1: find all joints in the image (organized by joint type: [0]=nose,
    # [1]=neck...)
    cdef int[:, :] joint_list_per_joint_type = NMS(param,
    ...
    cdef int[:, :, :] joint_list = np.array([tuple(peak) + (joint_type,) for joint_type,
