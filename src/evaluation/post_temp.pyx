...
cimport cv2
cimport matplotlib.cm
cimport numpy as np
from scipy.ndimage.filters cimport gaussian_filter, maximum_filter
from scipy.ndimage.morphology cimport generate_binary_structure
from data_process.coco_process_utils cimport BODY_PARTS
...
cdef object[tuple, ndim=1] joint_to_limb_heatmap_relationship = BODY_PARTS
...
cdef int NUM_JOINTS = 17
cdef int NUM_LIMBS = len(joint_to_limb_heatmap_relationship)


cpdef int[:, :] find_peaks(dict param, double[:, :] img):
    """
    ...
    cdef int[:, :] peaks_binary = (maximum_filter(img, footprint=generate_binary_structure(
...
cpdef int[:, :] compute_resized_coords(int[:, :] coords, int resizeFactor):
    """
...
cpdef float[:, :, :] NMS(dict param, double[:, :, :] heatmaps, float upsampFactor=1.0,
        bool bool_refine_center=True, bool bool_gaussian_filt=False):
    """
    ...
    cdef int[:, :, :] joint_list_per_joint_type = []
    cdef int cnt_total_joints = 0
    ...
    for joint in range(NUM_JOINTS):
        cdef double[:, :] map_orig = heatmaps[:, :, joint]
        cdef int[:, :] peak_coords = find_peaks(param, map_orig)
        cdef double[:, :] peaks = np.zeros((len(peak_coords), 4))
        for i, peak in enumerate(peak_coords):
            if bool_refine_center:
                cdef int x_min, y_min = np.maximum(0, peak - win_size)
                cdef int x_max, y_max = np.minimum(
                    np.array(map_orig.T.shape) - 1, peak + win_size)

                # Take a small patch around each peak and only upsample that
                # tiny region
                cdef double[:, :] patch = map_orig[y_min:y_max + 1, x_min:x_max + 1]
                cdef double[:, :] map_upsamp = cv2.resize(
                ...
                cdef int[:] location_of_max = np.unravel_index(
                    map_upsamp.argmax(), map_upsamp.shape)
                # Remember that peaks indicates [x,y] -> need to reverse it for
                # [y,x]
                cdef int[:] location_of_patch_center = compute_resized_coords(
                    peak[::-1] - [y_min, x_min], upsampFactor)
                # Calculate the offset wrt to the patch center where the actual
                # maximum is
                cdef int[:] refined_center = (location_of_max - location_of_patch_center)
                cdef double peak_score = map_upsamp[location_of_max]
            else:
                cdef int[:] refined_center = [0, 0]
                # Flip peak coordinates since they are [x,y] instead of [y,x]
                cdef float peak_score = map_orig[tuple(peak[::-1])]
...
cpdef float[:, :, :] find_connected_joints(dict param, float[:, :, :] paf_upsamp,
                                            float[:, :, :] joint_list_per_joint_type, int num_intermed_pts=10):
    """
    ...
    """
    cdef float[:, :, :] connected_limbs = []       # limb_type x num_all_limbs x 5

    # Auxiliary array to access paf_upsamp quickly
    cdef object[np.intp, ndim=2] limb_intermed_coords = np.empty((4, num_intermed_pts), dtype=np.intp)
    for limb_type in range(NUM_LIMBS):
        # List of all joints of type A found,
        # where A is specified by limb_type (eg: a right forearm starts in a right elbow)
        cdef float[:, :] joints_src = joint_list_per_joint_type[joint_to_limb_heatmap_relationship[limb_type][0]]
        # List of all joints of type B found, where B is specified by limb_type
        # (eg: a right forearm ends in a right wrist)
        cdef float[:, :] joints_dst = joint_list_per_joint_type[joint_to_limb_heatmap_relationship[limb_type][1]]
        ...
        if len(joints_src) == 0 or len(joints_dst) == 0:
            ...
            cdef list connected_limbs = []
            connected_limbs.append([])
        else:
            cdef float[:, :] connection_candidates = [[]]
            ...
            for i, joint_src in enumerate(joints_src):
                ...
                for j, joint_dst in enumerate(joints_dst):
                    ...
                    cdef int[:, :] limb_dir = joint_dst[:2] - joint_src[:2]
                    # Compute the distance/length of the potential limb (norm
                    # of limb_dir)
                    cdef int limb_dist = np.sqrt(np.sum(limb_dir ** 2)) + 1e-8
                    ...
                    cdef float[:, :, :] intermed_paf = paf_upsamp[limb_intermed_coords[0, :],
                                              limb_intermed_coords[1, :], limb_intermed_coords[2:4, :]].T

                    cdef float[:, :, :] score_intermed_pts = intermed_paf.dot(limb_dir)/(np.linalg.norm(intermed_paf)+1e-8)

                    cdef float score_penalizing_long_dist = score_intermed_pts.mean() + \
                                                             min(0.5 * paf_upsamp.shape[0] / limb_dist - 1, 0)
                    # Criterion 1: At least 80% of the intermediate points have
                    # a score higher than thre2
                    cdef float criterion1 = (np.count_nonzero(score_intermed_pts > param['thre2']) > 0.8 * num_intermed_pts)
                    # Criterion 2: Mean score, penalized for large limb
                    # distances (larger than half the image height), is
                    # positive
                    cdef bool criterion2 = (score_penalizing_long_dist > 0)
            ...
            cdef int[:, :] connections = np.empty((0, 5))
            ...
            # but 2 elbows)
            cdef int max_connections = min(len(joints_src), len(joints_dst))
            # Traverse all potential joint connections (sorted by their score)
            for potential_connection in connection_candidates:
                cdef int i, j = potential_connection[0:2]
                cdef float s = potential_connection[3]
...
cpdef object[np.intp, ndim=2] group_limbs_of_same_person(float[:, :, :] connected_limbs, float[:, :] joint_list):
    """
    Associate limbs belonging to the same person together.
    :param connected_limbs: See 'return' doc of find_connected_joints()     ---> N_L x N_detect x connections 
    :param joint_list: unraveled version of joint_list_per_joint [See 'return' doc of NMS()]
    ...
    """
    # matched edges
    cdef int[:, :] person_to_joint_assoc = [] # shape == N_person x (n_Joint + 2)

    for limb_type in range(NUM_LIMBS):
        cdef int joint_src_type, joint_dst_type = joint_to_limb_heatmap_relationship[limb_type]

        for limb_info in connected_limbs[limb_type]:
            cdef int[:] person_assoc_idx = []
            ...
            # associated to anyone yet:     ( close to KM algorithm )
            if len(person_assoc_idx) == 1:
                cdef int[:] person_limbs = person_to_joint_assoc[person_assoc_idx[0]]
                ...
                cdef int[:] person1_limbs = person_to_joint_assoc[person_assoc_idx[0]]
                cdef int[:] person2_limbs = person_to_joint_assoc[person_assoc_idx[1]]
                ...
                cdef int[:] row = -1 * np.ones(NUM_JOINTS+2)
                # Store the joint info of the new connection
                row[joint_src_type] = limb_info[0]      # peak_score
...
def decode_pose(double[:, :, :] img_orig, dict param, double[:, :, :] heatmaps, double[:, :, :] pafs):
    heatmaps = heatmaps.transpose(1,2,0)
    pafs = pafs.transpose(1,2,0)
    # Bottom-up approach:
    # Step 1: find all joints in the image (organized by joint type: [0]=nose,
    # [1]=neck...)
    cdef float[: ,:, :] joint_list_per_joint_type = NMS(param,
    ...
    cdef float[:, :] joint_list = np.array([tuple(peak) + (joint_type,) for joint_type,
    ...
    # with which elbows)
    cdef float[:, :, :] paf_upsamp = cv2.resize(
        pafs, (img_orig.shape[1], img_orig.shape[0]), interpolation=cv2.INTER_CUBIC)
    cdef float[:, :, :] connected_limbs = find_connected_joints(param,paf_upsamp, joint_list_per_joint_type)

    # Step 3: associate limbs that belong to the same person
    cpdef object[np.intp, ndim=2] person_to_joint_assoc = group_limbs_of_same_person(connected_limbs, joint_list)
...
cpdef append_result(int image_id, object[np.intp, ndim=2] person_to_joint_assoc, float[:, :] joint_list,
                    object[dict, ndim=1]outputs):
    ...
    for ridxPred in range(len(person_to_joint_assoc)):
        ...
        cdef object[np.intp, ndim=2] keypoints = np.zeros((17, 3))

        for part in range(17):
            cdef int ind = part
            cdef int index = int(person_to_joint_assoc[ridxPred, ind])
