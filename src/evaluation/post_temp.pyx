...
def find_peaks(dict param, double[:, :] img):
    """
...
def compute_resized_coords(int[:, :] coords, int resizeFactor):
    """
...
def NMS(dict param,double[:, :, :] heatmaps, float upsampFactor=1.,
...
def find_connected_joints(dict param, double[:, :, :] paf_upsamp, int[:, :, :, :] joint_list_per_joint_type, int num_intermed_pts=10):
    """
        ...
        if len(joints_src) == 0 or len(joints_dst) == 0:
            ...
            for i, joint_src in enumerate(joints_src):
                ...
                for j, joint_dst in enumerate(joints_dst):
                    ...
                    cdef double criterion1 = (np.count_nonzero(
...
def decode_pose(double[:, :, :] img_orig, dict param, double[:, :, :] heatmaps, double[:, :, :] pafs):
    ...
    # with which elbows)
    cdef double [:, :, :] paf_upsamp = cv2.resize(
