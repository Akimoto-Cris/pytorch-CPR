...
class process:
    ...
    def step(self, data_loader, criterion_hm, criterion_paf, to_train=False, optimizer=None, viz_output=False):
        if to_train:
            self.model.train()
        else:
            self.model.eval()
    ...
    def train_net(self, train_loader, test_loader, criterion_hm, criterion_paf, optimizer,
        ...
        for epoch in range(1, n_epochs + 1):
            ...
            if epoch % val_interval == 0:
                torch.cuda.empty_cache()
                with torch.no_grad():
                    self.validate_net(test_loader, criterion_hm, criterion_paf, save_dir, epoch, viz_output=viz_output)
